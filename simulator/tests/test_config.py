...
validation, enum membership), the example file we ship, and the
...
def test_load_minimal_valid(tmp_path: Path):
    ...
    assert cfg.broker == BrokerConfig(target=BrokerTarget.LOCAL, url="mqtt://localhost:1883")
...
def test_config_is_frozen(tmp_path: Path):
    """SimulatorConfig + nested dataclasses are frozen (caught accidentally
    mutating in early drafts)."""
    cfg = load_config(_write(tmp_path, VALID_YAML))
...
def test_non_healthy_scenarios_parse(tmp_path: Path, value: str):
    """Per ADR/plan: non-healthy scenarios are accepted at load time so
    manifests can be authored ahead of the runner. Today's runtime only
    wires HEALTHY behavior, but the YAML schema is stable from day one."""
...
def test_aws_iot_broker_target_parses(tmp_path: Path):
...
@pytest.mark.parametrize("bad_count", [0, -1, 51, 9999])
...
def _config_with_demo_mode(enabled: bool) -> SimulatorConfig:
    return SimulatorConfig(
        fleet=FleetConfig(pump_count=15, setpoint_rpm=1800.0, ambient_celsius=22.0, base_seed=0),
        scenario=ScenarioKind.HEALTHY,
        broker=BrokerConfig(target=BrokerTarget.LOCAL, url="mqtt://localhost:1883"),
        demo_mode=enabled,
    )
