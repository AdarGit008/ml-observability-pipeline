...
- [x] **Per-pump AWS-mode identity + Terraform fleet provisioning landed 2026-06-07 (ADR 0016).** `broker.tls` paths accept a literal `{pump_id}` token, expanded per pump by `Fleet.from_config` via `simulator.config.tls_for_pump` (plain `str.replace`; pathless-token configs unchanged — the 2026-05-27 single-pump smoke shape still works; the stashed publisher_factory expands too, so `FleetExpansion.add_pump` mints per-pump identity). Infra side: `infra/modules/iot_fleet` provisions 15 Things (bare `P-NN` == client_id, per ADR 0003), AWS-generated certs (keys in LOCAL-ONLY gitignored tfstate — custody trade recorded in ADR 0016), ONE shared policy scoped by `${iot:Connection.Thing.ThingName}`, and writes cert material to `simulator/.secrets/` at apply. Demo sequence: `docs/runbooks/aws-demo-day.md`. 13 new tests (`simulator/tests/test_tls_per_pump.py`). **Live-confirmed 2026-06-07:** 15/15 pumps connected first try (~0.2 s) against real AWS IoT Core; per-pump mTLS + Connect/Publish-own-topic policy scoping worked as designed (2026-06-07 session log).
...
**In — `simulator/config.yaml`.** Loader in `simulator/config.py` (pure schema validation: required-keys, unknown-keys-reject, type + range, conditional `broker.tls`). Deserializes to a frozen `SimulatorConfig`. `broker.tls` paths may carry the `{pump_id}` placeholder (ADR 0016) — expanded at runner construction, never in the loader (shape-only validation stands). `profiles_for(config)` produces the per-state `StateProfile` dict for `Pump`, applying the `demo_mode` HEALTHY-dwell override.
...
- **ADR 0016** — Terraform-managed IoT fleet: cert custody (keys in local-only state), shared thing-variable policy, bare `P-NN` Thing names, `{pump_id}` config templating. Supersedes the Console-provisioned posture from 2026-05-27.
- Likely future ADRs: YAML-parametric scenarios (deferred per ADR 0004 §"Follow-ups").
