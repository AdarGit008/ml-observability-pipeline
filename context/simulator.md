...
- [x] `simulator/config.yaml` loading + `demo_mode` HEALTHY-dwell shortcut landed 2026-05-25 (45 new tests, 75 total passing). See `docs/sessions/2026-05-25-simulator-config-yaml.md`. Schema in `simulator/config.py`; annotated example in `simulator/config.example.yaml`. PyYAML added as first runtime dep.
- [ ] MQTT publishing (paho-mqtt, asyncio). `broker.target` (`local` | `aws-iot`) is in the YAML schema; the client is not yet wired.
- [ ] Scenario scripting (seasonal drift, fleet expansion, real failure). `scenario` is in the YAML schema and accepts all four values; only `healthy` produces behavior today.
...
- **In:** `simulator/config.yaml` — see `simulator/config.example.yaml` for the canonical, annotated example. Loader in `simulator/config.py` enforces required-keys, unknown-keys-reject, type, and range validation; deserializes to a frozen `SimulatorConfig`. `profiles_for(config)` produces the per-state `StateProfile` dict for the `Pump` class, applying the `demo_mode` HEALTHY-dwell override.
