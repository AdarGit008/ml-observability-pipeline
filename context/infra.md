...
- [ ] Not yet: `modules/fleet_psi` — the fleet-PSI Lambda handler shipped 2026-06-10 (ADR 0018, `lambda_fleet_psi`), but its Terraform (EventBridge `rate(5 minutes)` + Lambda + scoped IAM: Query on table ARN + `sns:Publish` on topic ARN + scoped logs), drift-only build script (`scripts/build_fleet_psi.{ps1,sh}`: numpy + `shared/` + reference JSON, no sklearn/model.pkl), env wiring (`SNS_TOPIC_ARN`/`FLEET_SIZE`), and `aws_teardown.sh` sweep are deferred to a follow-on infra session.
