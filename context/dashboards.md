...
- ~~`pumps_reporting` short-snapshot drift~~ — **off-by-one found and fixed live 2026-06-07.** `FLEET_PUMP_IDS` enumerated `P-01..P-15` (1-indexed); real fleet is `P-00..P-14`, so the adapter queried a nonexistent P-15 and skipped P-00 → `pumps_reporting: 14`. Fixed in `dashboards_adapter/handler.py` + 7 test patches (17/17 pass); NOT redeployed before teardown → `pumps_reporting == 15` is a redeploy-verify at the next apply. The underlying `FLEET_SIZE`-duplicates-simulator-fleet-size single-source-of-truth concern stands (now demonstrated to bite); revisit if it recurs. NB: `lambda_fleet_psi` enumerates `P-01..P-NN` (1-indexed) for its pooled-window reads — same single-source-of-truth concern, a separate component (out of scope for the adapter; flag if a fleet session reopens).
